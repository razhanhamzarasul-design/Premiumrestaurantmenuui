...
const menuData: MenuItem[] = [
  ...
  { id: 'f10', nameEn: 'Crispy Strips', nameAr: 'ستربس دجاج', nameKu: 'ستریپسی مریشک', descriptionEn: 'Four pieces of spicy crispy chicken strips', descriptionAr: 'أربع قطع من ستربس الدجاج المقرمش', descriptionKu: 'چوار پارچە ستریپسی مریشکی کریسپی', price: 5.5, image: 'https://images.unsplash.com/photo-1562967914-608f82629710?w=600&q=80', category: 'fastfood' },
  ...
  { id: 'm7', nameEn: 'Shrimp Risotto', nameAr: 'ريزوتو الروبيان', nameKu: 'ریزۆتۆی مەیگوو', descriptionEn: 'Italian creamy rice with fresh prawns', descriptionAr: 'أرز إيطالي كريمي مع الروبيان الطازج', descriptionKu: 'برنجی کرێمی ئیتالی بە مەیگووی فرێش', price: 28, image: 'https://images.unsplash.com/photo-1534422298391-e4f8c170db0a?w=600&q=80', category: 'mains' },
  { id: 'm8', nameEn: 'Beef Tagliatelle', nameAr: 'تاغلياتيلي باللحم', nameKu: 'تاگلیاتیلی گۆشت', descriptionEn: 'Egg pasta with slow-cooked beef ragu', descriptionAr: 'باستا البيض مع راغو اللحم المطهو ببطء', descriptionKu: 'پاستای هێلکە بە ڕاگۆی گۆشتی برژاو', price: 21, image: 'https://images.unsplash.com/photo-1551183053-bf91a1d81141?w=600&q=80', category: 'mains' },
  { id: 'm9', nameEn: 'Grilled Sea Bass', nameAr: 'سيباس مشوي', nameKu: 'سیباسی برژاو', descriptionEn: 'Whole sea bass with herbs and garlic', descriptionAr: 'سمك سيباس كامل مع الأعشاب والثوم', descriptionKu: 'ماسی سیباسی تەواو بە گژوگیا و سیر', price: 34, image: 'https://images.unsplash.com/photo-1534604973900-c41ab4c287ba?w=600&q=80', category: 'mains' },
  { id: 'm10', nameEn: 'Peking Duck', nameAr: 'بط بكين', nameKu: 'مراوی پەکین', descriptionEn: 'Crispy skin duck with hoisin sauce', descriptionAr: 'بط مقرمش مع صلصة الهويسين', descriptionKu: 'مراوی کریسپی بە سۆسی تایبەت', price: 42, image: 'https://images.unsplash.com/photo-1518492104633-130d0cc84637?w=600&q=80', category: 'mains' },

  // Starters & Drinks & Desserts (10+)
...
export default function App() {
  ...
  const [fadeSplash, setFadeSplash] = useState(false); // بۆ فەید ئاوتکردنی لۆگۆکە
  const [selectedItem, setSelectedItem] = useState<MenuItem | null>(null);
  const [isFlying, setIsFlying] = useState(false);

  // Splash Screen Logic
  useEffect(() => {
    // دوای ١.٥ چرکە دەستبکە بە کاڵکردنەوە (Fade Out)
    const fadeTimer = setTimeout(() => setFadeSplash(true), 1500);
    // دوای ٢ چرکە بە تەواوی لۆگۆکە لابدە
  ...
  // Splash Screen UI (Updated for Smooth Exit)
  if (loading) {
    return (
      <div className={`fixed inset-0 bg-[#050505] flex flex-col items-center justify-center z-[100] transition-all duration-700 ease-in-out ${fadeSplash ? 'opacity-0 scale-110 pointer-events-none' : 'opacity-100 scale-100'}`}>
        <div className="relative">
          <div className="w-24 h-24 border-2 border-[#d4af37]/20 rounded-full animate-ping absolute inset-0"></div>
          <div className="w-24 h-24 border-t-2 border-[#d4af37] rounded-full animate-spin relative z-10"></div>
        </div>
        <div className={`mt-8 text-center transition-all duration-700 ${fadeSplash ? 'translate-y-10' : 'translate-y-0'}`}>
          <h1 className="text-4xl font-black text-[#d4af37] tracking-[0.4em] font-serif">ROYAL DINE</h1>
          <p className="text-gray-500 mt-4 text-[10px] tracking-[0.5em] uppercase">{t('Fine Dining Concept', 'مفهوم الطعام الفاخر', 'چەمکی خواردنی شاهانە')}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#050505] text-white selection:bg-[#d4af37] selection:text-black font-sans animate-in fade-in duration-1000" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <header className={`sticky top-0 z-50 h-16 transition-all duration-500 ${scrolled ? 'bg-[#0a0a0a]/95 backdrop-blur-md border-b border-[#d4af37]/20 shadow-2xl' : 'bg-[#050505]'}`}>
        <div className="max-w-md mx-auto px-6 h-full flex items-center justify-between">
          <button className="p-2.5 bg-white/5 rounded-full hover:bg-[#d4af37]/10 transition-colors"><MapPin className="w-5 h-5 text-[#d4af37]" /></button>
      ...
      <main className="max-w-md mx-auto px-4 py-2 pb-32">
        {categories.filter(c => selectedCategory === 'all' || c.id === selectedCategory).map(cat => {
          ...
          return (
            <div key={cat.id} className="mb-12 animate-in slide-in-from-bottom-4 duration-700">
              ...
              <div className="grid grid-cols-2 gap-4">
                {items.map(item => (
                  <div key={item.id} className="bg-[#0f0f0f] rounded-[2.5rem] overflow-hidden border border-white/5 flex flex-col group active:scale-95 transition-all duration-300 shadow-xl">
                    <div className="relative aspect-square cursor-pointer" onClick={() => setSelectedItem(item)}>
                      ...
                      <div className="absolute top-3 left-3 flex flex-col gap-2">
                        {item.isPopular && <div className="bg-black/60 p-2 rounded-full backdrop-blur-sm shadow-lg border border-[#d4af37]/20"><Star className="w-4 h-4 text-[#d4af37] fill-[#d4af37]" /></div>}
                        {item.isSpicy && <div className="bg-red-500/20 p-2 rounded-full backdrop-blur-sm shadow-lg border border-red-500/20"><Flame className="w-4 h-4 text-red-500 fill-red-500" /></div>}
                      </div>
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <button onClick={() => addToCart(item)} className="mt-auto w-full bg-white/5 hover:bg-[#d4af37] text-[#d4af37] hover:text-black py-3 rounded-2xl flex items-center justify-center gap-2 text-[11px] font-bold transition-all active:bg-white shadow-inner">
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-[70] bg-black/95 backdrop-blur-2xl flex items-center justify-center p-6 animate-in fade-in zoom-in duration-300">
          <div className="w-full max-w-sm bg-[#0a0a0a] rounded-[3.5rem] overflow-hidden border border-[#d4af37]/20 shadow-[0_0_80px_rgba(212,175,55,0.1)]">
            <div className="relative h-72">
              <img src={selectedItem.image} className="w-full h-full object-cover" alt="" />
              <button onClick={() => setSelectedItem(null)} className="absolute top-6 right-6 p-2.5 bg-black/50 rounded-full backdrop-blur-xl border border-white/10 hover:bg-[#d4af37] hover:text-black transition-colors"><X className="w-6 h-6" /></button>
            </div>
            <div className="p-8 space-y-5">
              <div className="flex justify-between items-start">
                <h2 className="text-2xl font-black leading-tight">{t(selectedItem.nameEn, selectedItem.nameAr, selectedItem.nameKu)}</h2>
      ...
      {cartOpen && (
        <div className="fixed inset-0 z-[60] bg-black/95 backdrop-blur-2xl flex items-end justify-center animate-in fade-in duration-500">
          <div className="w-full max-w-md h-[90vh] bg-[#050505] rounded-t-[4rem] flex flex-col border-t border-[#d4af37]/30 shadow-[0_-20px_80px_rgba(0,0,0,0.8)]">
            <div className="p-8 flex justify-between items-center border-b border-white/5">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">{totalItems} {t('items ready', 'أصناف جاهزة', 'خواردن ئامادەیە')}</p>
              </div>
              <button onClick={() => setCartOpen(false)} className="p-3 bg-white/5 rounded-full hover:bg-white/10 transition-colors"><X className="w-6 h-6" /></button>
            </div>
            <div className="flex-1 overflow-y-auto px-8 py-6">
              {cart.length === 0 ? (
                ...
                <div className="space-y-6">
                  {cart.map(item => (
                    <div key={item.id} className="flex gap-5 items-center bg-white/5 p-4 rounded-[2rem] border border-white/5 animate-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="p-8 bg-[#0a0a0a] rounded-t-[4rem] border-t border-white/10 space-y-6 shadow-2xl">
              ...
              <button 
                ...
                className={`w-full py-5 rounded-[2rem] font-black text-sm uppercase tracking-widest flex items-center justify-center gap-3 transition-all ${cart.length === 0 ? 'bg-gray-800 text-gray-600' : 'bg-green-600 text-white active:scale-95 shadow-xl shadow-green-900/30 hover:bg-green-500'}`}
              >
      ...
      <footer className="max-w-md mx-auto px-8 py-16 bg-[#0a0a0a] border-t border-white/5 rounded-t-[4rem] text-center space-y-10">
        ...
        <div className="flex gap-6 justify-center">
          <a href="#" className="p-4 bg-white/5 rounded-[1.8rem] hover:text-[#d4af37] transition-all hover:-translate-y-1"><Instagram className="w-6 h-6" /></a>
          <a href="#" className="p-4 bg-white/5 rounded-[1.8rem] hover:text-[#d4af37] transition-all hover:-translate-y-1"><Facebook className="w-6 h-6" /></a>
          <a href="tel:9647503358550" className="p-4 bg-white/5 rounded-[1.8rem] hover:text-[#d4af37] transition-all hover:-translate-y-1"><Phone className="w-6 h-6" /></a>
        </div>
